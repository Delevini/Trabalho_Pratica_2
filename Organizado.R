...
EScams <- ms_simplify(EScams, keep = 0.02, keep_shapes = TRUE)
ESdonk <- ms_simplify(ESdonk, keep = 0.02, keep_shapes = TRUE)
ESdiff <- ms_simplify(ESdiff, keep = 0.02, keep_shapes = TRUE)

meus_dados_prontos <- list(
  compara22 = compara22,
  compara23 = compara23,
  cams22 = cams22,
  cams23 = cams23,
...
saveRDS(meus_dados_prontos, "dados_dashboard.rds")
...
pacote_dados <- readRDS("dados_dashboard.rds")

# 2. A MÁGICA ACONTECE AQUI:
# Essa função pega o que está dentro da lista e joga para o ambiente global
list2env(pacote_dados, envir = .GlobalEnv)
